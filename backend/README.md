...
Since [@nestjs/axios](https://github.com/nestjs/axios) default return [Observable](https://rxjs.dev/guide/observable), it does not fit in our use case (Promise based), so we use a custom module to implement secondary encapsulation of native axios library, also extract .data from the response to prevent .data.data.data... chaining.
...
```json
{
  ...
  "fastify": "HTTP server/framework that NestJS based on [default is Express.js]",
  "joi": "Environment variable validation library",
